...
| ワークフロー | トリガー | 目的 |
|---|---|---|
| `Validate portal` | `main` 宛てのプルリクエスト、手動実行 | 型検査とGitHub Pages用ビルドによる公開前の検証 |
| `Deploy static site to GitHub Pages` | `main` へのpush、手動実行 | GitHub Pagesへの公開 |
| `Monthly official standards check` | 毎月1日 02:17 UTC、手動実行 | 公式情報の確認、確認日更新、要確認Issue、再公開 |

`main` への変更は、公開ワークフローで型検査とGitHub Pages用の静的ビルドを完了してから公開されます。プルリクエストでは公開せず、`Validate portal` により同じビルド条件で検証します。
